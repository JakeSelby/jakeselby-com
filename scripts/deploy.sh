...
# Prerequisites: AWS_PROFILE=cortex set in env (or set here)
set -euo pipefail

export AWS_PROFILE="${AWS_PROFILE:-cortex}"
...
# Capture outputs
BUCKET=$(cd infra && npx cdk output --stack JakeSelby BucketName --no-staging 2>/dev/null | tr -d '[:space:]' || echo "jakeselby-com-web")
